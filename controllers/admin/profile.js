...
router.route('/saved/:id').get(function(req, res) {
	Jobs.find({ userId: req.user.id }, function(err, result) {
		if (err) {
			...
			User.find({ type: 'candidate' }, function(err, obj) {
				if (err) {
...
router.route('/save/candidate/:jobId/:candidateId').get(function(req, res) {
...
router.route('/remove/candidate/:jobId/:candidateId').get(function(req, res) {
	Jobs.findOneAndUpdate({ _id: req.params.jobId }, { $pull: { candidateId: req.params.candidateId } }, function(
		err,
		obj
	) {
		if (err) {
...
router.route('/remove/saved-candidate/:jobId/:candidateId').get(function(req, res) {
	Jobs.findOneAndUpdate({ _id: req.params.jobId }, { $pull: { candidateId: req.params.candidateId } }, function(
		err,
		obj
	) {
...
router.post('/company/block', function(req, res) {
	if (req.user.type == 'candidate') {
		var blockList = req.body.blockList.filter(function(e) {
			return e;
		});
